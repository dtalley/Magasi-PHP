...
*/
	...
	class sys {
      ...
      self::hook( "terminating_message" );
        ...
        if( $code == USER_ERROR || $code == AUTHENTICATION_ERROR ) {
        ...
        action::end();
				tpl::revert();
				echo tpl::load( "util/message", 0, true );
			}
			exit();
		}
    ...
    public static function timezone() {
      if( self::$timezone == NULL ) {
        $timezone = preferences::get( "global", "timezone", "account" );
        if( !$timezone ) {
          $timezone = action::get( "settings/default_timezone" );
        }
    ...
    public static function error_in_maintenance( $errno, $errstr, $errfile, $errline ){
      $fp = fopen( "maintenance.html", 'r' );
      echo fread( $fp, filesize( "maintenance.html" ) );
      fclose( $fp );
      exit;
    }
    
    public static function exception_in_maintenance( $exception ){
      $fp = fopen( "maintenance.html", 'r' );
      echo fread( $fp, filesize( "maintenance.html" ) );
      fclose( $fp );
      exit;
