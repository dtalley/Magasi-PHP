...
*/
  ...
  require_once( DATABASE_DIR . "/database.php" );
    ...
    private static $duration = array();
      ...
      if( self::$query_open ) {
        ...
        if( !$result = mysql_query( $sql, self::$connection ) ) {
          return false;
        }
        if( self::$debug ) {
          $end = microtime(true);
    ...
    public static function result() {
			...
			if( isset( self::$result[self::$current_result] ) ) {
				if( !$val = mysql_fetch_array( self::$result[self::$current_result] ) ) {
          self::clear_result();
				}
				return $val;
			} else {
    ...
    public static function clear_result() {
      if( self::$current_result >= 0 ) {
        if( isset( self::$result[self::$current_result] ) ) {
          mysql_free_result( self::$result[self::$current_result] );
        }
  ...
  class dbquery {
    ...
    public function loose_link( $column1, $column2, $level ) {
			...
			$alias = $table['alias'];
			$this->where( $column1, $alias . "." . $column2, "", "", false );
		}
    ...
    public function select_sql() {
			...
			$this->parse_select( $this->index, $select, $from, $where );
      ...
      if( $from != " FROM " ) {
        $sql .= $from;
			...
			if( $this->limit ) {
				$sql .= " LIMIT " . $this->limit['start'] . ", " . $this->limit['total'];
			}
      if( $this->encased ) {
        $sql = "( " . $sql . " )";
      }
      if( $this->union_query != NULL ) {
		...
		private function parse_select( $index, &$select, &$from, &$where ) {
        ...
        if( isset( $this->children[$index[$i]['index']] ) ) {
          $this->parse_select( $this->children[$index[$i]['index']], $select, $from, $where );
        }
